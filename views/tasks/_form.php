...
//            'template' => "{label}:\n".User::getNameById($model->user_id)."\n{hint}\n{error}"] )
    ...
    <?= $form->field($model, 'order_opencart_id')->dropDownList(

        \app\models\Orders::getFullInfoOrderItemsForDropDownList(),
        [
        'prompt'=>'Выберите заказ',
        'onchange'=>'
                $.get( "index.php?r=tasks/get-next-number-task&idOrder="+$(this).val()+"&idTask="+$("#tasks-id").val(), function( rawData ) {
